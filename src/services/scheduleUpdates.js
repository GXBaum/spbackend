...
export function scheduleUpdates({
    ...
    const englishRoomJobHardCodedTuesday = scheduler.scheduleJob("25 13 * * 2", async () => {
        englishHardcodedIds.forEach((id) => {
            sendNotificationToUser(id, "Englisch im Raum 155", "(nicht an den VP angepasst)")
        })
    });
    const englishRoomJobHardCodedWednesday = scheduler.scheduleJob("25 13 * * 3", async () => {
        englishHardcodedIds.forEach((id) => {
            sendNotificationToUser(id, "Englisch im Raum 216", "(nicht an den VP angepasst)")
        })
    });
    const englishRoomJobHardCodedThursday = scheduler.scheduleJob("50 7 * * 4", async () => {
        englishHardcodedIds.forEach((id) => {
            sendNotificationToUser(id, "Englisch im Raum 209", "(nicht an den VP angepasst)")
        })
    });
