...
export function createCourseRepository(db) {
  ...
  const stmts = {
    insertCourse: db.prepare(`INSERT OR IGNORE INTO sp_course (id, name) VALUES (?, ?)`),
    insertUserCourse: db.prepare(`INSERT OR IGNORE INTO user_sp_course (user_id, course_id) VALUES (?, ?)`),
    getUserCourses: db.prepare(`SELECT course_id FROM user_sp_course WHERE user_id = ?`),
    getUserCourseNames: db.prepare(`
      ...
      ORDER BY s.name
    `),
    selectAllCourses: db.prepare(`SELECT id, name FROM sp_course`),
  };
