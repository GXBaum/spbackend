...
export function createVpRepository(db) {
  ...
  const stmts = {
      // vp_difference
      getPlanHtml: db.prepare(`SELECT data FROM vp_difference WHERE day = ?`),
    ...
    insertSubstitution: db.prepare(`
      ...
      VALUES (?,?,?,?,?,?,?)
    `),
    deleteVpSubstitutionsForDay: db.prepare(`DELETE FROM vp_substitution WHERE day = ?`),
    deleteVpSubstitutionsForCourse: db.prepare(`DELETE FROM vp_substitution WHERE course = ?`),
    getLatestVpDate: db.prepare(`SELECT vp_date FROM vp_substitution ORDER BY vp_date DESC LIMIT 1`),

    // user selected courses
    insertUserVpSelectedCourse: db.prepare(`
      INSERT OR IGNORE INTO user_vp_course (user_id, course) VALUES (?, ?)
    `),
    deleteUserVpSelectedCourse: db.prepare(`
      DELETE FROM user_vp_course WHERE user_id = ? AND course = ?
    `),
      getUserVpSelectedCourses: db.prepare(`
      SELECT course FROM user_vp_course WHERE user_id = ? ORDER BY course
    `),
    getUsersWithVPCourseName: db.prepare(`
      SELECT user_id FROM user_vp_course WHERE course = ?
    `),
  ...
  VALUES (?, ?)
`),
      getLatestVpInfo: db.prepare(`SELECT * FROM vp_info WHERE day = ? ORDER BY fetched_at DESC LIMIT 1`),
      getVpInfoHistory: db.prepare(`SELECT * FROM vp_info WHERE day = ? ORDER BY fetched_at ASC`),
  };
  ...
  return {
    ...
    listSubstitutionsForCourses(courses, day, vpDate) {
      ...
      const sql = `
        SELECT course, hour, original, replacement, description, vp_date
        FROM vp_substitution
        WHERE course IN (${placeholders}) AND day = ? AND vp_date = ?
