...
export function createMarkRepository(db) {
  ...
  const stmts = {
    deleteMarksOfHalfYear: db.prepare(`DELETE FROM sp_mark WHERE user_id = ? AND half_year = ?`),
    insertMark: db.prepare(`
      INSERT OR IGNORE INTO sp_mark (name, date, grade, course_id, user_id, half_year)
      VALUES (?, ?, ?, ?, ?, ?)
    `),
    getUserMarks: db.prepare(`SELECT * FROM sp_mark WHERE user_id = ? ORDER BY date DESC`),
    getUserMarksForCourse: db.prepare(`
      SELECT * FROM sp_mark WHERE user_id = ? AND course_id = ? ORDER BY date DESC
    `),
  };
