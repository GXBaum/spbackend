...
export function createUserRepository(db) {
    ...
    const stmts = {
        insertUser: db.prepare(`INSERT INTO user (notifications_enabled) VALUES (?)`),
        updateUserNotifications: db.prepare(`UPDATE user SET notifications_enabled = ? WHERE id = ?`),
        getUserById: db.prepare(`SELECT * FROM user WHERE id = ?`),
        getUsersWithEnabledNotifications: db.prepare(`SELECT id FROM user WHERE notifications_enabled = 1`),
